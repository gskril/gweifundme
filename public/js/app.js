...
;(async () => {
	...
	// Check if the donation address has a primary ENS name
	if (
		!destinationEns.includes('0x') &&
		!destinationEns.includes('...')
	) {
		try {
			...
			if (Object.values(ensRecords).every((value) => value === null)) {
				...
				if (ensRecords.github) {
					...
					githubEl.classList.add(
						'profile__link',
						'profile__link--github'
					)
		...
		console.log('Not checking for ENS records')
	}

	try {
		// lookup past transactions for destinationAddress after block 14130427 (first gweifundme txn)
		const history  = await etherscanProvider.getHistory(destinationAddress, 14130427)
